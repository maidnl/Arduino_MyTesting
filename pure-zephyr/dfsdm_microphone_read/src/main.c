...
{
	/* 1. Ensure the SYSCFG clock is enabled so we can read its registers */
	__HAL_RCC_SYSCFG_CLK_ENABLE();

	/* 2. Read the hardware registers */
	uint32_t pwr_d3cr = PWR->D3CR;
	uint32_t syscfg_pwrcr = SYSCFG->PWRCR;

	/* 3. Extract the specific bits */
...
void print_ahb_clock(void)
{
	/* HAL_RCC_GetHCLKFreq() directly reads the hardware registers
	 * and calculates the exact running frequency in Hertz.
	 */
	uint32_t hclk_freq = HAL_RCC_GetHCLKFreq();

	printk("\n========================================\n");
	printk("DIAGNOSTIC: AHB (SRAM) Clock Speed\n");
	printk("Frequency: %u Hz (%u MHz)\n", hclk_freq, hclk_freq / 1000000);
	printk("========================================\n\n");
}
void dump_system_config()
{
	printk("========================================\n");
	printk("       SYSTEM CONFIGURATION DUMP        \n");
	printk("========================================\n");

	// 1. Power and Voltage Scaling (VOS)
	// PWR_D3CR controls the Voltage Scaling (VOS) level.
	printk("PWR_D3CR (Voltage Scale) : 0x%08X\n", PWR->D3CR);

	// 2. Flash Memory Latency
	// FLASH_ACR controls read wait states.
	printk("FLASH_ACR (Latency)      : 0x%08X\n", FLASH->ACR);

	// 3. Core Clocks and Prescalers
	// These control the exact dividers for the AHB and APB buses.
	printk("RCC_D1CFGR (AHB Presc)   : 0x%08X\n", RCC->D1CFGR);
	printk("RCC_D2CFGR (APB1/2 Presc): 0x%08X\n", RCC->D2CFGR);
	printk("RCC_D3CFGR (APB3/4 Presc): 0x%08x\n", RCC->D3CFGR);

	// 4. Power Supply Configuration
	// PWR_CR3 controls whether the LDO or SMPS is used.
	printk("PWR_CR3 (Power Supply)   : 0x%08X\n", PWR->CR3);
